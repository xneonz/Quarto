    ...
    protected String moveSelectionAlgorithm(int pieceID) {
        ...
        for(int i = 0; i < this.quartoBoard.getNumberOfRows(); i++) {
            for(int j = 0; j < this.quartoBoard.getNumberOfColumns(); j++) {
                if(!this.quartoBoard.isSpaceTaken(i, j)) {
                    QuartoBoard winningCopyBoard = new QuartoBoard(this.quartoBoard);
                    winningCopyBoard.insertPieceOnBoard(i, j, pieceID);
                    if(winningCopyBoard.checkRow(i) || winningCopyBoard.checkColumn(j) || winningCopyBoard.checkDiagonals()) {
                        return i + "," + j;
                    }
                }
            }
